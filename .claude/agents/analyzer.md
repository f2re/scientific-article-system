...
description: Analyzes meteorology, NWP, and ML papers using 10-point relevance scoring. Extracts methodology, datasets, metrics, and results into structured JSON. Triggers on: new PDFs in papers/, batch analysis requests, systematic reviews.
...
- Save analysis in `analysis/` with date and topic
- Organize findings per `papers/` structure
- Create comparison tables for batch analysis
- Use tools (Read, Grep, Glob) for efficient navigation

## Proactive Workflow

When new PDFs detected in `papers/`:
1. Automatically initiate relevance analysis
2. Extract methodology and results
3. Save structured JSON to `analysis/YYYY-MM-DD_[topic]/`
4. Generate summary table if multiple papers

For systematic reviews:
1. Group papers by theme
2. Create comparative analysis across studies
3. Identify literature gaps
4. Suggest search directions
